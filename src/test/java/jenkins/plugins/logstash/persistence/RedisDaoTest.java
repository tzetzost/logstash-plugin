...
public class RedisDaoTest {
  ...
  RedisDao createDao(String host, int port, String key, String username, String password, String proxyHost, int proxyPort) {
    return new RedisDao(mockPool, host, port, key, username, password, proxyHost, proxyPort);
  }
  ...
  public void constructorFailNullHost() throws Exception {
    try {
      createDao(null, 6379, "logstash", "username", "password", "", -1);
  ...
  public void constructorFailEmptyHost() throws Exception {
    try {
      createDao(" ", 6379, "logstash", "username", "password", "", -1);
    } catch (IllegalArgumentException e) {
      assertEquals("Wrong error message was thrown", "host name is required", e.getMessage());
      throw e;
    }
  ...
  public void constructorFailNullKey() throws Exception {
    try {
      createDao("localhost", 6379, null, "username", "password", "", -1);
    } catch (IllegalArgumentException e) {
  ...
  public void constructorFailEmptyKey() throws Exception {
    try {
      createDao("localhost", 6379, " ", "username", "password", "", -1);
    } catch (IllegalArgumentException e) {
      assertEquals("Wrong error message was thrown", "redis key is required", e.getMessage());
      throw e;
    }
  ...
  public void constructorSuccess() throws Exception {
    // Unit under test
    dao = createDao("localhost", 6379, "logstash", "username", "password", "", -1);
  ...
  public void pushSuccessNoAuth() throws Exception {
    ...
    // Initialize mocks
    dao = createDao("localhost", 6379, "logstash", null, null, "", -1);
    when(mockJedis.rpush("logstash", json)).thenReturn(1L);
